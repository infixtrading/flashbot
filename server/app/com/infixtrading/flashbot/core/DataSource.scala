...
abstract class DataSource(dataTypes: Set[String]) {
  ...
  def discoverTopics(exchangeConfig: Option[ExchangeConfig])
    ...
    Future.successful(exchangeConfig.map(_.pairs.map(_.toString)).getOrElse(Set.empty))
...
object DataSource {
  ...
  final case class DataSourceConfig(`class`: String, topics: Set[String], dataTypes: Set[String])
