...
//  def qty: Double = math.abs(tn.toDouble(amount))
  ...
  def map(fn: Double => Double): FixedSize = FixedSize(fn(amount), security)
}
...
//  implicit def mkNumericOps(t: FixedSize): FixedSizeFractional.FractionalOps = FixedSizeFractional.mkNumericOps(t)
//  implicit def mkOrderingOps(t: FixedSize): FixedSizeFractional.Ops = FixedSizeFractional.mkOrderingOps(t)

  implicit class ConvertFixedSizeOps(size: FixedSize) {
    def as[T: AssetKey](key: T)(implicit prices: PriceIndex,
                                instruments: InstrumentIndex,
                                metrics: Metrics): FixedSize = {
      as(key, strict = false)
    }

    def as[T: AssetKey](key: T, strict: Boolean)
