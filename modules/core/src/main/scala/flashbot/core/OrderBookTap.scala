...
object OrderBookTap {
  ...
  def simpleLadderSimulation(): Source[Ladder, NotUsed] = {
    val initialLadder: Ladder = new Ladder(25, 1)
    ...
    Source(Stream.from(0))
      .scan(initialLadder) {
        case (ladder, _) =>
          ...
          val delta = Math.abs(Gaussian(0, 5).draw().intValue())
          val price = quoteSide.makeWorseBy(referencePrice, delta)

//          println(s"Side: ${quoteSide}\tPrice: ${price}\tQty at price: ${ladder.qtyAtPrice(price)}")
...
//          TableUtil.renderLadder(ladder, 5)
          ...
          if (Random.nextInt(10) == 0) {
            val tradeAmt = Gaussian(10, 2).draw().intValue()
