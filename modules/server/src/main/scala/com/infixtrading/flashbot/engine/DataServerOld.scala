...
//    Props(new DataServerOld(
//      new File(fbConfig.`jdbc-url`),
//      fbConfig.sources,
//      fbConfig.exchanges,
//      ingestConfig = None,
//      useCluster = false))
...
//}
//
//class DataServerOld(marketDataPath: File,
...
//    Source(sources)
//      .zipWithIndex
//      .flatMapConcat { case (src, i) => src.map(md => (s"${i}_${md.bundle}", md)) }
//
